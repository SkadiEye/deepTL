...
SEXP backprop_BLOCK(List n_hidden, double w_ini,
  ...
  NumericVector n_node_total(n_layer);
  // Rcout << n_hidden.size() << endl;
  for(int i = 0; i < n_layer + 1; i++) {

    // Rcout << i << endl;
    // Rcout << as<NumericVector>(n_hidden(i+1)) << endl;
    // Rcout << as<NumericVector>(n_hidden(i+1)).size() << endl;
    // Rcout << (as<NumericVector>(n_hidden(i+1))).size() << endl;
    // Rcout << (as<vec>(n_hidden(i+1))).size() << endl;
    // if(as<vec>(n_hidden(i+1)).size() > 1) {

      // Rcout << as<vec>(n_hidden(i+1)) << endl;
    if(i < n_layer)
      n_node_total[i] = sum(as<vec>(n_hidden(i+1)));
    // }
    // else {

      // Rcout << as<double>(n_hidden(i+1)) << endl;
      // n_node_total[i] = as<double>(n_hidden(i+1));
    // }
    // Rcout << n_node_total << endl;
    if(i == 0) {
      if(load_param) {
        ...
        // weight_(i) = (randu<mat>(x_.n_cols, n_node_total[i]) - 0.5) * w_ini * 2;
        bias_(i) = (randu<vec>(n_node_total[i]) - 0.5) * w_ini;
        if(as<vec>(n_hidden(i+1)).size() > 1) {

          weight_(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
          int start_row = 0;
          int end_row = as<vec>(n_hidden(i))[0] - 1;
          int start_col = 0;
          int end_col = as<vec>(n_hidden(i+1))[0] - 1;
          // Rcout << start_row << ' ' << start_col << ' ' << end_row << ' ' << end_col << endl;
          weight_(i).submat(start_row, start_col, end_row, end_col) =
            (randu<mat>(as<vec>(n_hidden(i))[0], as<vec>(n_hidden(i+1))[0]) - 0.5) * w_ini * 2;
          for(unsigned int j = 1; j < (as<vec>(n_hidden(i+1)).size()); j++) {

            start_row = end_row + 1;
            end_row += as<vec>(n_hidden(i))[j];
            start_col = end_col + 1;
            end_col += as<vec>(n_hidden(i+1))[j];
            // Rcout << start_row << ' ' << start_col << ' ' << end_row << ' ' << end_col << endl;
            weight_(i).submat(start_row, start_col, end_row, end_col) =
              (randu<mat>(as<vec>(n_hidden(i))[j], as<vec>(n_hidden(i+1))[j]) - 0.5) * w_ini * 2;
          }
        } else {

          weight_(i) = (randu<mat>(n_node_total[i-1], n_node_total[i]) - 0.5) * w_ini * 2;
        }
      }
      dw(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
      db(i) = vec(n_node_total[i], fill::zeros);
    } else if(i == n_layer) {
      if(load_param) {
        ...
        weight_(i) = (randu<mat>(n_node_total[i-1], 1) - 0.5) * w_ini * 2;
        bias_(i) = (randu<vec>(1) - 0.5) * w_ini;
      }
      dw(i) = mat(n_node_total[i-1], 1, fill::zeros);
      ...
      if(load_param) {
        ...
        if(as<vec>(n_hidden(i+1)).size() > 1) {
          ...
          weight_(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
          int start_row = 0;
          int end_row = as<vec>(n_hidden(i))[0] - 1;
          int start_col = 0;
          int end_col = as<vec>(n_hidden(i+1))[0] - 1;
          ...
          // Rcout << n_node_total[i-1] << ' ' << n_node_total[i] << endl;
          weight_(i) = (randu<mat>(n_node_total[i-1], n_node_total[i]) - 0.5) * w_ini * 2;
          // Rcout << n_node_total[i-1] << ' ' << n_node_total[i] << endl;
        }
      }
      dw(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
      db(i) = vec(n_node_total[i], fill::zeros);
    }
  }
  ...
  if(learning_rate_adaptive == "momentum") {
    ...
    for(int i = 0; i < n_layer + 1; i++) {
      ...
      if(i == 0) {
        last_dw(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        last_db(i) = vec(n_node_total[i], fill::zeros);
      } else if(i == n_layer) {
        last_dw(i) = mat(n_node_total[i-1], 1, fill::zeros);
        last_db(i) = vec(1, fill::zeros);
      } else {
        last_dw(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        last_db(i) = vec(n_node_total[i], fill::zeros);
      }
    }
    ...
    for(int i = 0; i < n_layer + 1; i++) {
      ...
      if(i == 0) {
        weight_ss(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        bias_ss(i) = vec(n_node_total[i], fill::zeros);
      } else if(i == n_layer) {
        weight_ss(i) = mat(n_node_total[i-1], 1, fill::zeros);
        bias_ss(i) = vec(1, fill::zeros);
      } else {
        weight_ss(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        bias_ss(i) = vec(n_node_total[i], fill::zeros);
      }
    }
    ...
    for(int i = 0; i < n_layer + 1; i++) {
      ...
      if(i == 0) {
        weight_egs(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        bias_egs(i) = vec(n_node_total[i], fill::zeros);
        weight_es(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        bias_es(i) = vec(n_node_total[i], fill::zeros);
      } else if(i == n_layer) {
        weight_egs(i) = mat(n_node_total[i-1], 1, fill::zeros);
        bias_egs(i) = vec(1, fill::zeros);
        weight_es(i) = mat(n_node_total[i-1], 1, fill::zeros);
        bias_es(i) = vec(1, fill::zeros);
      } else {
        weight_egs(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        bias_egs(i) = vec(n_node_total[i], fill::zeros);
        weight_es(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        bias_es(i) = vec(n_node_total[i], fill::zeros);
      }
    }
    ...
    for(int i = 0; i < n_layer + 1; i++) {
      ...
      if(i == 0) {
        mt_w(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        mt_b(i) = vec(n_node_total[i], fill::zeros);
        vt_w(i) = mat(x_.n_cols, n_node_total[i], fill::zeros);
        vt_b(i) = vec(n_node_total[i], fill::zeros);
      } else if(i == n_layer) {
        mt_w(i) = mat(n_node_total[i-1], 1, fill::zeros);
        mt_b(i) = vec(1, fill::zeros);
        vt_w(i) = mat(n_node_total[i-1], 1, fill::zeros);
        vt_b(i) = vec(1, fill::zeros);
      } else {
        mt_w(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        mt_b(i) = vec(n_node_total[i], fill::zeros);
        vt_w(i) = mat(n_node_total[i-1], n_node_total[i], fill::zeros);
        vt_b(i) = vec(n_node_total[i], fill::zeros);
      }
    }
  }
