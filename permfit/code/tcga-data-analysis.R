...
y <- factor(dat$tcga$y, labels = c("A", "B"))
x <- as.matrix(dat$tcga$x)
...
rf_vanilla_feature <- which(1 - pnorm(rf_mod$importance[, "MeanDecreaseAccuracy"]/
                                        rf_mod$importanceSD[, "MeanDecreaseAccuracy"]) < 0.1)
...
for(k in 1:k_fold) {
  ...
  pred[validate, 1] <- predict(dnn_mod, x[validate, ])[, "A"]

  #### 2.2 PermFIT-DNN ####
  dnn_mod <- ensemble_dnnet(importDnnet(x = x[-validate, dnn_feature], y = y[-validate]), n_ensemble, esCtrl, verbose = 0)
  pred[validate, 2] <- predict(dnn_mod, x[validate, dnn_feature])[, "A"]

  #### 2.3 SVM ####
  svm_mod <- tune.svm(x[-validate, ], y[-validate], gamma = 10**(-(0:4)), cost = 10**(0:4/2), tunecontrol = tune.control(cross = 5))
  svm_mod <- svm(x[-validate, ], y[-validate], gamma = svm_mod$best.parameters$gamma, cost = svm_mod$best.parameters$cost, probability = TRUE)
  pred[validate, 3] <- attr(predict(svm_mod, x[validate, ], decision.values = TRUE, probability = TRUE), "probabilities")[, "A"]

  #### 2.4 PermFIT-SVM ####
  svm_mod <- tune.svm(x[-validate, svm_feature], y[-validate], gamma = 10**(-(0:4)), cost = 10**(0:4/2), tunecontrol = tune.control(cross = 5))
  svm_mod <- svm(x[-validate, svm_feature], y[-validate], gamma = svm_mod$best.parameters$gamma, cost = svm_mod$best.parameters$cost, probability = TRUE)
  pred[validate, 4] <- attr(predict(svm_mod, x[validate, svm_feature], decision.values = TRUE, probability = TRUE), "probabilities")[, "A"]

  #### 2.5 RFE-SVM ####
  svm_mod <- tune.svm(x[-validate, svm_rfe_feature], y[-validate], gamma = 10**(-(0:4)), cost = 10**(0:4/2), tunecontrol = tune.control(cross = 5))
  svm_mod <- svm(x[-validate, svm_rfe_feature], y[-validate], gamma = svm_mod$best.parameters$gamma, cost = svm_mod$best.parameters$cost, probability = TRUE)
  pred[validate, 5] <- attr(predict(svm_mod, x[validate, svm_rfe_feature], decision.values = TRUE, probability = TRUE), "probabilities")[, "A"]

  #### 2.6 RF ####
  rf_mod <- randomForest(x[-validate, ], y[-validate], ntree = 1000, importance = TRUE)
  pred[validate, 6] <- predict(rf_mod, x[validate, ], type = "prob")[, 1]

  #### 2.7 PermFIT-RF ####
  rf_mod <- randomForest(x[-validate, rf_feature], y[-validate], ntree = 1000, importance = TRUE)
  pred[validate, 7] <- predict(rf_mod, x[validate, rf_feature], type = "prob")[, 1]

  #### 2.8 Vanilla-RF ####
  rf_mod <- randomForest(x[-validate, rf_vanilla_feature], y[-validate], ntree = 1000, importance = TRUE)
  pred[validate, 8] <- predict(rf_mod, x[validate, rf_vanilla_feature], type = "prob")[, 1]
}
...
data.frame(method = c("DNN", "PermFIT-DNN", "SVM", "PermFIT-SVM", "RFE-SVM", "RF", "PermFIT-RF", "Vanilla-RF"),
           accuracy = apply(pred, 2, function(x) acc(y = y, x)),
           AUC = apply(pred, 2, function(x) auc(y = y, x)))
